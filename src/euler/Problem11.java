...
public class Problem11 {
    ...
    public static int[][] numbers = new int[][]{
        ...
        {0, 0, 0, 86, 56, 00, 48, 35, 71, 89, 7, 5, 44, 44, 37, 44, 60, 21, 58, 51, 54, 17, 58, 0, 0, 0},
    ...
    public static void main(String[] args) {
        int greatestProduct = 0, east = 0, south = 0, diagonalleftdown = 0, diagonalrightdown = 0;

        for (int i = 3; i <= 22; i++) {
            for (int j = 3; j <= 22; j++) {
                int temp = getGreatestProduct(i, j);
                if (temp > greatestProduct) {
                    System.out.println("new greatestproduct = " + temp);
    ...
    private static int getGreatestProduct(int i, int j) {
        int result = 0;
        result = findEastProduct(i, j);
        int temp2 = findSouthProduct(i, j);
        if (temp2 > result) {
            result = temp2;
        }
        int temp3 = findDiagonalDownRightProduct(i, j);
        if (temp3 > result) {
            result = temp3;
        }
        int temp4 = findDiagonalDownLeftProduct(i, j);
        if (temp4 > result) {
            result = temp4;
        }
        return result;
    ...
    private static int findEastProduct(int x, int y) {
        int result = 1;
        for (int i = 0; i < 4; i++) {
            result = (result * (numbers[x][y + i]));
        }
        return result;
    }

    private static int findSouthProduct(int x, int y) {
        int result = 1;
        for (int i = 0; i < 4; i++) {
            result = (result * (numbers[x + i][y]));
        }
        return result;
    }

    private static int findDiagonalDownRightProduct(int x, int y) {
        int result = 1;
        for (int i = 0; i < 4; i++) {
            result = (result * (numbers[x + i][y + i]));
        }
        return result;
    }

    private static int findDiagonalDownLeftProduct(int x, int y) {
        int result = 1;
        for (int i = 0; i < 4; i++) {
            result = (result * (numbers[x + i][y - i]));
        }
